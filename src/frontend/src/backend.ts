...
export type backendInterface = {
  ...
  // Auto ML Pattern Store
  putMLPatterns(json: string): Promise<undefined>;
  getMLPatterns(): Promise<string>;
};
...
const idlFactory = ({ IDL }: { IDL: any }) => {
  return IDL.Service({
    ...
    putMLPatterns: IDL.Func([IDL.Text], [], []),
    getMLPatterns: IDL.Func([], [IDL.Text], ["query"]),
  });
};
